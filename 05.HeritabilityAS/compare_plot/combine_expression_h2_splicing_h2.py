...
expr_file = 'rrBLUP.output.gene_expression.xls'
sp_gtf_file_format = 'fromGTF.%s.txt'
sp_file_format = 'rrBLUP.%s_PSI.xls'
geneID_link_file = 'geneID.correspond.txt'
output = 'gene_expr_splice.herit.xls'
output_txt = ''
    ...
    for line in f:
        ...
        if dic_event_gene[gene]:
            for sp in dic_event_gene[gene]:
                if sp in dic_sp_h:
                    output_txt += '%s\t%s\t%s\t%s\n' % (gene,h,dic_sp_h[sp],sp.split('_')[0])
        else:
            output_txt += '%s\t%s\t0\tNO\n' % (gene,h)
