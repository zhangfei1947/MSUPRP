...
PSI <- read.table(paste0(type,"_PSI.filter.txt"),head=T,row.names=1)
SNP <- read.table("SNP.txt",head=T,row.names=1)
trans <- read.table(paste0(type,".trans.eqtl.xls.FDR_0.001"),head=T,row.names=1)
cis <- read.table(paste0(type,".cis.eqtl.xls.FDR_0.001"),head=T,row.names=1)
