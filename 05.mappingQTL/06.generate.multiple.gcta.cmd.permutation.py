...
cmd = '''gcta64 --bfile msuprp --grm-sparse sp_msuprp --fastGWA-mlm --pheno {file} --mpheno {N} --covar fixed.txt --threads 1 --out {gene} 
'''
...
for namefile in namefiles:
    ...
    for eachname in open(namefile,"r").read().strip("\n").split("\n"): 
        ...
        if (i%1000) == 0:
            cmd_txt += '''
for gwafile in *fastGWA
do
    awk 'BEGIN{ORS=",";print"\\n'${gwafile/.fastGWA/}'"}$10<0.01{print$10}' ${gwafile} >> AS.pvalue.txt 
done
rm *fastGWA *log
'''
