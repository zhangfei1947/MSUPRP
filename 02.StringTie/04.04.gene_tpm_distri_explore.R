...
for (plot_cv in c(0.1, 0.2, 0.3, 0.4, 1, 3)){
	for (plot_ske in c(5, 3, 2.5, 2, 1, 0.5)){
		genetpm_p = filter(genetpm,CV<(plot_cv*1.1) & CV>(plot_cv*0.9) & skewness<(plot_ske*1.1) & skewness>(plot_ske*0.9))
		print(plot_cv)
		print(plot_ske)
		print(nrow(genetpm_p))
		if(nrow(genetpm_p)>2){
			p = genetpm_p[sample(nrow(genetpm_p),3),]
			for(j in 1:3){
				d = p[j,]
				png(paste0("CV_",plot_cv,"_skewness_",plot_ske,"_",j,".gene.tpm.png"),width=600,height=500,units="px")
